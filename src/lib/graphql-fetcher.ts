...
export async function graphqlFetcher<T = unknown>(
  ...
  if (typeof window !== 'undefined' && !options?.skipTracking) {
    ...
    if (json.data) {
      ...
      else if (data.Citisignal_productFacets || data.facets || queryName.includes('Facet') || queryName.includes('Search')) {
        source = 'search';
      }
    ...
    const demoInspectorStore = (window as Window & { 
    ...
    // Track the query
    const demoInspectorTrack = (window as Window & { 
